...
> *She knew which would whisper, and which one would groom.*
...
- **L12** — *groom* is the verb in [Maxwell's indictment and conviction](https://en.wikipedia.org/wiki/Trial_of_Ghislaine_Maxwell). Reads as nursery word on the surface (bridegroom / horse-groom), reads as the literal procurement term on the second pass. *Which would whisper* = the older girl's guiding/conditioning of the new one; *which one would groom* = the pyramid-graduation, the abused becoming the recruiter — the standard structure of trafficking networks, documented in Maxwell's own case. The line collapses comfort and recruitment into the same act. The stanza now carries two inheritance lines: Robert → Ghislaine (stanza 1, *pale stone*), and older recruit → new recruit (stanza 3, *which one would groom*). They rhyme.
