...
## Timeline quality bar — every record and blurb must pass

Before a record or blurb ships, it clears these four checks. They are the standing inspection standard; a reel ingest is not done until its new `claim` text and any new/edited blurb pass all four. Reader-facing surfaces = chapter `<h3>` titles + nav labels (`index.html`), narrative blurbs (`chapters/narrative.json`), and each record's `claim`. (`notes`, `_comment`, `sources[]` are backstage — exempt.)

1. **No source-video reference in reader-facing text.** The single most-violated rule on ingest. A `claim` that fact-checks the reel must NOT say "the reel claims X" / "the surfacing reel" / "an Instagram reel posted by @handle" / "per the reel" / "described in the reel" / an embedded `instagram.com/reel/…` URL. Present the underlying claim directly and assess it: "The claim that X… is false/unsupported", or attribute to a real named speaker/venue already in the record (a podcast, a person) — never to the prompting post. This is a restatement of the gallery-only rule above, applied at the `claim` level. Grep new claims for `reel|the post|the video|@\w` before committing. (A real person's actual handle that is itself a documented fact — e.g. a defendant's username in a conviction — may stay; a creator/source handle may not.)
2. **Header matches content.** The chapter `<h3>` must describe what its cards actually cover. When a card lands under a chapter whose title only names the first card's topic (e.g. a "Bretton Woods" chapter that also carries an unrelated Tesla-papers card), broaden the title or move the card. Don't leave an empty anchor (a chapter shell with zero records renders blank).
3. **Define acronyms and insider terms on first reader-facing use.** Each `claim` renders standalone — gloss the term *in that card*, not relying on a sibling card. Universally known (US, FBI, CIA, DNA, NATO) need no gloss; agency/program/PAC initialisms and codenames do: NDAA, CUFI, ADL, ATF, FOIA, PSI, GFC, mRNA, VAERS, CBER/CDER, NIST, OCHA, UNRWA, PFLP-GC, SBIR, UEI, AGI, DMFI/RJC, etc. Drop raw database strings (UUIDs, UEI codes) from reader text — they live in `notes`. Never use a record id as link text.
4. **Link the evidence to why it matters — in few words, flat voice.** The blurb orients (the why-this-is-here, the what-comes-below); the card does the citation work. Don't restate the card's dates/numbers in the blurb. Cut the Claude-voice tells: "the receipts are below" / "the record below" pointers, rhetorical kickers ("the through-line", "the contrast is the point"), em-dashes used as dramatic beats, staccato escalating fragments, setup-then-reveal "gotcha" turns, "we/us/our", and conclusions stated in the site's own voice (an official account is attributed, never adopted — see "No official story as ground truth").

To inspect the whole timeline at once, fan the reader-facing fields out to subagents grouped by anchor (header + blurb + every card under it), each running these four checks with identical wording so the bar is applied consistently.
