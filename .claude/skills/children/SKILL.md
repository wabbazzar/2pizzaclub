...
- **Page layout:** each scene = a full-bleed **two-up** (two illustration panels split by a diagonal `clip-path` seam), verse overlaid on soft scrims, placed on each image's darkest region via **`drafts/illustrations/darkest.py`**. The clip is on an inner `.art` div so the verse is **never chopped** by the seam. Narrow widths get a scrim back + reflow to a vertical stack.
