...
5. **Trace the clip through the still's hosting breadcrumbs.** Reverse-searching the fuller still surfaces video-CDN thumbnails (e.g. `1a-1791.com`, `image.24vids.com`) whose paths embed the source tweet/post ID (`tw-<id>`, `amplify_video_thumb/<id>`), and burned-in **creator watermarks ("@handle") name the source account** — follow either to the original post and `yt-dlp` it. When the source post is deleted or the CDN hotlink-403s the `.mp4`, the **thumbnail poster itself can be higher-res (2000px+) than any grabbable video** — clean its annotations and deliver it as the still, and say plainly the clean *video* needs the live post URL.
...
**One scene, many clips — confirm framing, not just event.** A viral "moment" usually has *several* distinct attendee videos shot from adjacent positions (worked case: a "white hat guy" clip and a "black shirt guy" clip of the same tent, seconds apart). Identifying/grabbing *one* doesn't mean it's the framing your query shows — a strong inlier match on the *shared* scene can still be the wrong camera that crops your subject out of frame entirely. Before delivering, verify the clip actually contains your query's subject (here: the right-edge man), not just the same banners and bystanders.
